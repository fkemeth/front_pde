...
from utils import Additive_Network, Model, progress
...
def main(config):
    ...
    network = Additive_Network(ex_cfg.config["MODEL"], n_vars=dataset_train.x_data.shape[1],
    ...
    if ex_cfg.config["TRAINING"]['proceed_training']:
        model.load_network(ex_cfg.config["boundary_conditions"]+'_add_test.model')
    ...
    for epoch in progress_bar:
        ...
        model.save_network(ex_cfg.config["boundary_conditions"]+'_add_test.model')
